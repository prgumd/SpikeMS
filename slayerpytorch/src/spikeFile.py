...
class event():
	...
	def __init__(self, xEvent, yEvent, pEvent, tEvent):
		...
		# print(self.x.shape, self.y.shape, self.p.shape, self.t.shape)
	...
	def toSpikeTensor(self, emptyTensor, samplingTime=1, randomShift=False, binningMode='OR'):	# Sampling time in ms
		...
		if self.dim == 1:
			...
			# print("emptyTensor shape", emptyTensor.shape)
			# print("tEvent shape", tEvent.shape)

			yEvent = np.round(self.y).astype(int)
			validInd = np.argwhere((pEvent < emptyTensor.shape[0]) &
									(yEvent < emptyTensor.shape[1]) & 
									(xEvent < emptyTensor.shape[2]) &
								   (tEvent < emptyTensor.shape[3]) &
								   (xEvent >= 0) &
								   (yEvent >= 0) & 
								   (pEvent >= 0) &
